...
db.restaurants.updateOne({name: "Panini Land"},
    {$push: {rating: {feedback: "Great Value", value: 5, consumer_id: consumer_id} } })

db.restaurants.find({name:"Panini Land"}, {rating:{consumer_id:1}})
...
db.drivers.insertOne({
    ...
    tools: [{type:"vehicle", name:"bike"},{type:"allowed", name:"bevrage"}],
    current_pos: ""
})

db.consumers.updateOne({_id: consumer_id},
    {$push: {orders: {
                items: [],
                total: 6.2,
                status: "complete",
                driver: driver_id,
                delivery_address: {loc:"Isep NDC 28"},
